...
- **A settings save or a toolbar button appears to do nothing?** Fenced writes used to fail silently. They now surface as a red line in the panel (an HTTP 403 names the fence and points at **Trusted hosts**), and the extension's most recent ack — `kind`, `opened`, `timeout`, `error` — is exposed as `lastAck` on `GET /__dsh-vsceditor/state`. `opened: false, timeout: true` means VS Code did not confirm the diff within 6s; `edit-error` means it threw, and the panel shows the reason too. `bridgeDebug` turns on the extension's own trace (`/tmp/dsh-bridge-debug.log`) when you need to go deeper
