...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    var CSS =
      ...
      '.dsh-vsced-acterr{color:#f85149;font-size:12px;max-width:720px;margin-top:6px;word-break:break-word}' +
    ...
    var L10N = {
      zh: {
        ...
        'tb.openWindow': '新窗口打开',
        'tb.restart': '重启',
        'tb.powerOn': '打开 code-server',
        'tb.powerOff': '关闭 code-server',
        'tb.powerOnLocal': '恢复连接',
        'tb.powerOffLocal': '断开连接',
        'empty.closed': '编辑器后端已关闭',
        'empty.closedHint': '当前未连接任何后端，code-server 已停止以释放内存。需要时按需启动：',
        'empty.closedStartCs': '▶ 启动内嵌 code-server',
        'empty.closedUseLocal': '连接本机 VS Code →',
        ...
        'settings.port': '端口',
        'settings.portHint': '0 = 随机（10000–65000）；改动会自动重启编辑器',
        'act.failed': '操作失败（{action}）：{error}',
        'act.rejected': '操作被拒绝（{action}）：HTTP 403 —— 写操作没通过控制面围栏，请检查你访问 DSH 用的主机名是否已加入 trustedHosts',
        'settings.bridgeDebug': '扩展调试追踪',
        'settings.bridgeDebugHint': '把扩展打开 diff 的过程写进 /tmp/dsh-bridge-debug.log；排查「没弹 diff」时用，即时生效',
        ...
        'settings.trustedHosts': '信任的主机',
        'settings.trustedHostsPh': '留空 = 仅回环；如 dsh.example.com, 192.168.1.5:61430',
        'settings.trustedHostsHint': '经反向代理 / 自定义域名访问 DSH 时在此声明主机名（裸 host 或 host:port，逗号分隔）；不声明则控制接口只接受本机回环访问。若本卡片已无法保存，请直接编辑 ~/.dsh/settings.yaml 的 dsh-vsceditor 节',
      ...
      en: {
        ...
        'tb.restart': 'Restart',
        'tb.powerOn': 'Start code-server',
        'tb.powerOff': 'Stop code-server',
        'tb.powerOnLocal': 'Reconnect',
        'tb.powerOffLocal': 'Disconnect',
        'empty.closed': 'Editor backend is off',
        'empty.closedHint': 'No backend connected; code-server is stopped to free memory. Start on demand:',
        'empty.closedStartCs': '▶ Start embedded code-server',
        'empty.closedUseLocal': 'Connect local VS Code →',
        ...
        'settings.port': 'Port',
        'settings.portHint': '0 = random (10000–65000); changes restart the editor',
        'act.failed': 'Action failed ({action}): {error}',
        'act.rejected': 'Action rejected ({action}): HTTP 403 — the write did not pass the control fence; check whether the hostname you use to reach DSH is listed in trustedHosts',
        'settings.bridgeDebug': 'Extension debug tracing',
        'settings.bridgeDebugHint': 'Append the diff-open trace to /tmp/dsh-bridge-debug.log; use it when a diff does not show up (takes effect immediately)',
        ...
        'settings.trustedHostsPh': 'empty = loopback only; e.g. dsh.example.com, 192.168.1.5:61430',
        'settings.trustedHostsHint': 'Declare the hostname you use to reach DSH through a reverse proxy or custom domain (bare host or host:port, comma separated); without it the control routes accept loopback only. If this card can no longer save, edit the dsh-vsceditor section of ~/.dsh/settings.yaml directly',
      ...
      'pt-BR': {
        ...
        'tb.powerOn': 'Iniciar code-server',
        'tb.powerOff': 'Parar code-server',
        'tb.powerOnLocal': 'Reconectar',
        'tb.powerOffLocal': 'Desconectar',
        'empty.closed': 'Backend do editor desligado',
        'empty.closedHint': 'Nenhum backend conectado; o code-server está parado para liberar memória. Inicie sob demanda:',
        'empty.closedStartCs': '▶ Iniciar code-server integrado',
        'empty.closedUseLocal': 'Conectar VS Code local →',
        ...
        'settings.port': 'Porta',
        'settings.portHint': '0 = aleatória (10000–65000); mudanças reiniciam o editor',
        'act.failed': 'Ação falhou ({action}): {error}',
        'act.rejected': 'Ação rejeitada ({action}): HTTP 403 — a escrita não passou pela cerca de controle; verifique se o hostname usado para acessar o DSH está em trustedHosts',
        'settings.bridgeDebug': 'Rastreamento de depuração da extensão',
        'settings.bridgeDebugHint': 'Grava o rastro de abertura de diff em /tmp/dsh-bridge-debug.log; use quando um diff não aparece (efeito imediato)',
        ...
        'settings.trustedHosts': 'Hosts confiáveis',
        'settings.trustedHostsPh': 'vazio = apenas loopback; ex.: dsh.example.com, 192.168.1.5:61430',
        'settings.trustedHostsHint': 'Declare o hostname usado para acessar o DSH por proxy reverso ou domínio próprio (host ou host:port, separados por vírgula); sem isso as rotas de controle aceitam apenas loopback. Se este cartão não conseguir mais salvar, edite a seção dsh-vsceditor de ~/.dsh/settings.yaml',
      ...
      es: {
        ...
        'tb.restart': 'Reiniciar',
        'tb.powerOn': 'Iniciar code-server',
        'tb.powerOff': 'Detener code-server',
        'tb.powerOnLocal': 'Reconectar',
        'tb.powerOffLocal': 'Desconectar',
        'empty.closed': 'Backend del editor apagado',
        'empty.closedHint': 'Sin backend conectado; code-server está detenido para liberar memoria. Inicia bajo demanda:',
        'empty.closedStartCs': '▶ Iniciar code-server incrustado',
        'empty.closedUseLocal': 'Conectar VS Code local →',
        ...
        'settings.portHint': '0 = aleatorio (10000–65000); los cambios reinician el editor',
        'act.failed': 'La acción falló ({action}): {error}',
        'act.rejected': 'Acción rechazada ({action}): HTTP 403 — la escritura no pasó la valla de control; comprueba si el hostname con el que accedes a DSH está en trustedHosts',
        'settings.bridgeDebug': 'Traza de depuración de la extensión',
        'settings.bridgeDebugHint': 'Escribe el rastro de apertura de diff en /tmp/dsh-bridge-debug.log; úsalo cuando un diff no aparece (efecto inmediato)',
        ...
        'settings.trustedHosts': 'Hosts de confianza',
        'settings.trustedHostsPh': 'vacío = solo loopback; p. ej. dsh.example.com, 192.168.1.5:61430',
        'settings.trustedHostsHint': 'Declara el hostname con el que accedes a DSH mediante proxy inverso o dominio propio (host o host:port, separados por comas); sin esto las rutas de control solo aceptan loopback. Si esta tarjeta ya no puede guardar, edita la sección dsh-vsceditor de ~/.dsh/settings.yaml',
    ...
    // 写操作失败不再静默。此前 postAction 是 .catch(() => null)：被围栏拒绝时
    // 面板毫无反应，只是 2.5s 后轮询把控件弹回原值，用户会以为「点了没生效」。
    var actionError = null
    var actionErrorListeners = []
    function setActionError(next) {
      actionError = next
      for (var i = 0; i < actionErrorListeners.length; i++) {
        try { actionErrorListeners[i](next) } catch (e) {}
      }
    }
    function useActionError() {
      var s = React.useState(actionError)
      React.useEffect(function () {
        var f = function (v) { s[1](v) }
        actionErrorListeners.push(f)
        return function () {
          var i = actionErrorListeners.indexOf(f)
          if (i >= 0) actionErrorListeners.splice(i, 1)
        }
      }, [])
      return s[0]
    }
    function actionErrorText(lang, err) {
      if (!err) return ''
      // 403 只可能来自控制面围栏，直接把排查方向写进提示里。
      if (err.status === 403) return t(lang, 'act.rejected', { action: err.action || '-' })
      return t(lang, 'act.failed', { action: err.action || '-', error: err.message || '' })
    }
    function postAction(body) {
      var name = (body && body.action) || ''
      return fetch(ACTION_URL, {
        ...
        // 业务失败（{ok:false}）仍把结果交给调用方，同时把原因挂到横幅上。
        if (j && j.ok === false) setActionError({ action: name, status: 0, message: String(j.error || '') })
        else setActionError(null)
        return j
      }).catch(function (e) {
        setActionError({ action: name, status: (e && e.status) || 0, message: String((e && e.message) || e) })
        return null
      })
    }
    ...
    function statusOf(st) {
      ...
      // 关闭态：灰灯，优先级高于后端细分状态（此时不连接任何后端）
      if (st.backend === 'off' || (st.config && st.config.editorBackend === 'off')) return { color: '#888', text: t(lang, 'st.closed') }
    ...
    function EditorView() {
      ...
      // 关闭态：backend=off，不连接任何后端
      var isOff = !!(st && (st.backend === 'off' || (st.config && st.config.editorBackend === 'off')))
      ...
      React.useEffect(function () {
        // 关闭态：释放 iframe 渲染进程内存（code-server 已停，留着也是死页面）
        if (isOff && frame) { frameUrl = ''; frame.src = 'about:blank' }
      ...
      children.push(React.createElement('div', { className: 'dsh-vsced-toolbar', key: 'tb' },
        ...
        !isLocal && !isOff && url ? React.createElement('a', { className: 'dsh-vsced-btn', href: url, target: '_blank', rel: 'noreferrer' }, t(lang, 'tb.openWindow')) : null,
        !isLocal && !isOff ? React.createElement('button', { className: 'dsh-vsced-btn', onClick: function () { postAction({ action: 'restart' }) } }, t(lang, 'tb.restart')) : null,
        // 电源按钮：与设置卡片里「编辑器后端 = 关闭」互斥项联动同一个配置位。
        // 关闭态下变成「打开 code-server」（回到内嵌后端）。
        React.createElement('button', {
          className: 'dsh-vsced-btn',
          onClick: function () {
            postAction({ action: 'set-config', patch: { editorBackend: isOff ? 'embedded' : 'off' } })
          },
        }, isOff ? t(lang, 'tb.powerOn') : (isLocal ? t(lang, 'tb.powerOffLocal') : t(lang, 'tb.powerOff')))
      ))
      if (actionErr) {
        children.push(React.createElement('div', { className: 'dsh-vsced-acterr', key: 'acterr' }, actionErrorText(lang, actionErr)))
      }
      if (isOff) {
        // 关闭态引导：按需启动内嵌 code-server 或连接本机 VS Code
        var closedError = hostText(st, 'lastErrorCode', 'lastError', lang)
        children.push(React.createElement('div', { className: 'dsh-vsced-empty', key: 'off' },
          React.createElement('div', null, t(lang, 'empty.closed')),
          React.createElement('div', { className: 'hint' }, t(lang, 'empty.closedHint')),
          React.createElement('div', { className: 'row', style: { justifyContent: 'center' } },
            React.createElement('button', {
              className: 'dsh-vsced-btn',
              style: { fontWeight: 600 },
              onClick: function () { postAction({ action: 'set-config', patch: { editorBackend: 'embedded' } }) },
            }, t(lang, 'empty.closedStartCs')),
            React.createElement('button', {
              className: 'dsh-vsced-btn',
              onClick: function () { postAction({ action: 'set-config', patch: { editorBackend: 'local' } }).then(function () { openWizard(st) }) },
            }, t(lang, 'empty.closedUseLocal'))
          ),
          closedError ? React.createElement('pre', { style: { maxWidth: '100%', overflow: 'auto', fontSize: 11, color: '#f85149', whiteSpace: 'pre-wrap' } }, closedError) : null
        ))
      } else if (isLocal) {
    ...
    function SettingsCard() {
      ...
      function saveTrustedHosts(ev) {
        var v = ev.target.value.trim()
        if (v !== (cfg.trustedHosts || '')) save({ trustedHosts: v })
      }
      ...
      return React.createElement('li', { className: 'dsh-vsced-card' + (open ? ' dsh-vsced-card-open' : '') },
        ...
        !open ? null : React.createElement('div', { className: 'dsh-vsced-cardbody' },
          ...
          React.createElement('div', { className: 'row' },
            React.createElement('span', null, t(lang, 'settings.trustedHosts')),
            React.createElement('input', {
              key: 'th' + (cfg.trustedHosts || ''),
              type: 'text',
              defaultValue: cfg.trustedHosts || '',
              placeholder: t(lang, 'settings.trustedHostsPh'),
              style: { flex: 1, minWidth: '220px' },
              onBlur: saveTrustedHosts, onKeyDown: enterBlur,
            })
          ),
          React.createElement('div', { className: 'row' },
            React.createElement('span', { className: 'hint' }, t(lang, 'settings.trustedHostsHint'))
          ),
          React.createElement('div', { className: 'row' },
            ...
            // 第三互斥项：关闭。选中后 host 停掉 code-server、摘掉 bridge.json，
            // 所有自动拉起路径被 startServer 的 off 守卫挡住（见 host.js）。
            React.createElement('label', { className: 'row', style: { gap: '4px' } },
              React.createElement('input', {
                type: 'radio', name: 'dsh-vsced-backend',
                checked: cfg.editorBackend === 'off',
                onChange: function () { save({ editorBackend: 'off' }) },
              }),
              t(lang, 'settings.backendOff')
            )
          ),
          ...
          // 诊断开关：host 通过 SSE 的 debug 帧即时下发，扩展无需重启即可开始
          // 追踪「打开 diff」的过程（桌面 VS Code 窗口也一样，见 host.js）。
          React.createElement('label', { className: 'row' },
            React.createElement('input', {
              type: 'checkbox',
              checked: !!cfg.bridgeDebug,
              onChange: function (e) { save({ bridgeDebug: e.target.checked }) },
            }),
            t(lang, 'settings.bridgeDebug'),
            React.createElement('span', { className: 'hint' }, t(lang, 'settings.bridgeDebugHint'))
          ),
          cfg.editorBackend !== 'embedded' ? null : React.createElement('label', { className: 'row' },
            React.createElement('input', {
              type: 'checkbox',
          ...
          cfg.editorBackend !== 'embedded' ? null : React.createElement('div', { className: 'row' },
          ...
          cfg.editorBackend !== 'embedded' ? null : (st && (st.install && st.install.phase === 'running' || missingCodeServer(st)))
            ? React.createElement('div', { className: 'row' },
              React.createElement('button', {
                className: 'dsh-vsced-btn',
                onClick: function () { openInstallWizard(st) },
          ...
          st && st.settingsAvailable === false
            ...
            : null,
          actionErr
            ? React.createElement('div', { className: 'dsh-vsced-acterr' }, actionErrorText(lang, actionErr))
            : null,
          lastError
