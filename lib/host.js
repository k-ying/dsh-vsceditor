...
// Rules (browser pages cannot forge Host/Origin):
//   - Host must be loopback or an IP literal (a DNS-rebinding page carries a
//     DOMAIN Host and fails here; IP literals cannot be rebound)
...
function hostIsDirect(hostname) {
...
function isTrustedControlRequest(req, mutation) {
  const host = req.headers && req.headers.host
  if (typeof host !== 'string' || host === '') return false
  let hostUrl
  try { hostUrl = new URL('http://' + host) } catch (e) { return false }
  if (!hostIsDirect(hostUrl.hostname)) return false
...
const plugin = {
  ...
  apply(ctx, config) {
    ...
    disposers.push(webServer.register({
      ...
      path: CONTROL_STATE,
      handler(req, res) {
        if (!isTrustedControlRequest(req, false)) { res.writeHead(403); res.end(); return }
    ...
    disposers.push(webServer.register({
      ...
      path: CONTROL_ACTION,
      handler(req, res) {
        if (!isTrustedControlRequest(req, true)) { res.writeHead(403); res.end(); return }
        captureBrowserLang(req)
        readBody(req, 65536, (body) => {
          try {
            const msg = JSON.parse(body || '{}')
